...
def get_sales_invoice_id_from_doc_id(id):
	return frappe.db.sql(
		"""
		select transportation_invoice
...
def get_meta(doc):
	...
	query = {
		'name': doc['id'] if doc['type'] in ('Consignment Note',) else get_sales_invoice_id_from_doc_id(doc['id'])
	}

	return_doc = frappe.db.sql("""
